...
main();  
  ...
  function main(){
    ...
    if (templ.layers.length<2){
        alert('В файле шаблона должно быть как минимум два слоя. \nСкрипт не может продолжать работу');
        return;
        }
    ...
    for (var i=templ.layers.length-1;i>=0;i--){
          ...
          if (folderWithName == null) {
               alert('Не найдена папка с именем как у слоя: ' + templ.layers[i].name + '\nСкрипт не может продолжать работу');
               return;
              }
           if (folderWithName.getFiles(/\.(ai|eps)$/i).length==0) {
               alert('Папка' + templ.layers[i].name + ' не содержит файлов .eps или .ai \nСкрипт не может продолжать работу');
               return;
              }
     ...
     if (baseLayerCount!=1) {
               alert('Один и только один слой должен быть основным и иметь имя начинающееся с символа 0. \nСкрипт не может продолжать работу');
               return;
              }
