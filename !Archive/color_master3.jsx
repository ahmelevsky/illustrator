...
for (var a = 0; a < doc.layers.length; a++){
    ...
    if (groupItems.length > maxgroupscount)
        maxgroupscount = groupItems.length;
    var iSet= [];
...
var minсolorscount;
for (var sw = 1; sw < app.activeDocument.swatchGroups.length; sw++){
    var сolorscount = app.activeDocument.swatchGroups[sw].getAllSwatches().length;
    if (sw==1)
        minсolorscount=сolorscount;
    if (minсolorscount>сolorscount)
        minсolorscount=сolorscount;
}

 if (isUnique && maxgroupscount>minсolorscount) {
  alert ("Ошибка! В некоторых палитрах цветов всего: " + minсolorscount + ", а максимальное количество групп в слоях достигает " + maxgroupscount);
        return;     
     }
 ...
 if (isUnique)
            cSets = getColorsSets(doc, maxgroupscount, countforeachswgroup);
 else
            cSets = getColorsSetsAny(doc, maxgroupscount, countforeachswgroup);
