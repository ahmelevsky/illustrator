...
function main_container() {
    ...
    var legacyParam = app.preferences.getBooleanPreference("fileFormatGetFile/ConvertedInFilename");
    if (legacyParam) {
        app.preferences.setBooleanPreference("fileFormatGetFile/ConvertedInFilename", false)
    }
    try {
        main();
    } catch (exp) {
    ...
    if (legacyParam) {
        app.preferences.setBooleanPreference("fileFormatGetFile/ConvertedInFilename", true)
    }
...
function metaXerox_only() {
    ...
    var rootfolder = folderObj.selectDlg("Только копирование метаданных! Выбери корневую папку в которой лежат файлы");
    if (!rootfolder) {
        return null;
    }
    var allfolders = [];
...
function main() {
    ...
    var folderObj = new Folder();
    if (params.work_path) {
        var rootfolder = folderObj.selectDlg("Выбери корневую папку с файлами", params.work_path)
    } else {
        var rootfolder = folderObj.selectDlg("Выбери корневую папку с файлами")
    }
    if (!rootfolder) {
        return null;
    }
    ...
    if (!eps10T_template.exists) {
        ...
        var eps10T_template = new File(scriptPath + slash + "template_for_reExport_EPS.eps");
    }
    if (!eps10T_template.exists) {
        var eps10T_template = new File(Folder.desktop + slash + "template_for_reExport_EPS.eps");
    }
    if (!eps10T_template.exists) {
        alert("Файл-шаблон template_for_reExport_EPS.eps должен быть или рядом с файлом скрипта, или на рабочем столе, или в выбранной корневой папке. Без этого работать не будет. Может быть везде; у файла в папке с обрабатываемыми всегда приоритет.");
        return null;
    }
    if (params.target_mp) {
        var target_MP = params.target_mp
    } else {
        var target_MP = prompt("Подгонять размер под 4МП или больше? Максимум 25\n2000:2000 = 4МП, 5000:5000 = 25МП\nПо умолчанию 6МП - небольшое превышение на всякий случай", 6, "Параметры ресайза")
    }
    ...
    if (target_MP > 25000000) {
        alert("Целевой размер должен быть не более 25МП");
        return null;
    }
    if (!target_MP) {
    ...
    for (var j = 0; j < allfolders.length; j++){
        ...
        for (var i = 0; i < files.length; i += 1) {
            try {
                reExport(files[i], targetFolder, slash, target_MP, eps10T_template, params);
            } catch (exp) {
                alert(exp);
                errors++;
            }
...
function reExport(fileName, targetFolder, slash, target_MP, eps10T_template, params) {
    ...
    var keywords = read_prop_from_file(fileName.fsName.replace(".eps", ".jpg"), "subject");
    if (title == "") {
        title = ReadXMPMetadata("title")
    }
    if (description == "") {
        description = ReadXMPMetadata("description")
    }
    if (keywords == "") {
        keywords = ReadXMPMetadata("subject")
    }
    if (params.copy_jpg) {
    ...
    clippath.name = "temp_empty_clipping_path_sholud_be_removed";
    if (params.pixel_perfect) {
        var zoom = 100 * Math.ceil(Math.sqrt(target_MP / (clippath.width * clippath.height)));
    } else {
    ...
    if (params.unmask) {
        ...
        app.selection = null;
        app.redraw();
        epsDoc.layers[0].pathItems.temp_empty_clipping_path_sholud_be_removed.remove();
        app.redraw();
        app.executeMenuCommand("selectall");
        ...
        epsDoc.layers[0].groupItems[0].pathItems.temp_empty_clipping_path_sholud_be_removed.name = ""
    }
    app.selection = null;
    app.redraw();
    fitView();
...
function read_ini() {
    ...
    var scriptFile = new File($.fileName);
    var scriptPath = scriptFile.path;
    var iniFile = new File(scriptPath + slash + "reEPS_2019.ini");
    iniFile.open("r");
    while (!iniFile.eof) {
        var line = trim(iniFile.readln());
        if (line != "" && line[0] != "#") {
            ...
            if (key == "work_path") {
                value = trim(line[1]).toLocaleLowerCase()
            } else if (key == "target_mp") {
                value = trim(line[1]).replace(",", ".")
            } else if (key == "pixel_perfect") {
                if (trim(line[1]).toLocaleLowerCase() == "yes") {
                    value = true
                }
            } else {
                if (trim(line[1]).toLocaleLowerCase() == "no") {
                    value = false
                }
            }
            params[key] = value;
